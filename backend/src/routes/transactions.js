...
router.post("/", auth, async(req, res) => {
    try {
        ...
        if (Number.isNaN(Date.parse(transaction_date))) {
        ...
        const categoryCheck = category_id ?
            await pool.query(
                "SELECT id FROM categories WHERE id = $1 AND user_id = $2", [category_id, req.user.id]
            ) :
            { rowCount: 1 };
...
router.delete("/:id", auth, async(req, res) => {
    const result = await pool.query(
        "DELETE FROM transactions WHERE id=$1 AND user_id=$2 RETURNING id", [req.params.id, req.user.id]
    );
